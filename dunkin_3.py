...
Original file is located at
    https://colab.research.google.com/drive/129bh2nRw7R_Tz8KGTNFQ5aNuR8i9gMZO
...
The following code defines some **global variables**, meaning that they will be accessible in the rest of the code blocks below.  It also imports a couple of functions from library modules.  And it defines a couple of helper functions (that should look familar) that you can use in writing your code.
"""
...
# They will help you index into product tuples to retrieve the type and size of that product.
...
As we learned in notebook 3, a dictionary is a data structure in P]ython that consists of "keys" and their associated "values". There is no limit on the number of keys or values a dictionary can hold. For example, you may have dict = {'Teacher': 'Prof. Fallis', 'Students': ['Marta', 'Kevin', 'Ryan']}. In this dictionary, the keys are 'Teacher' and 'Students' and the values are 'Prof. Fallis', which is attached to the key 'Teacher', and ['Marta', 'Kevin', 'Ryan'], which is attached to the key 'Students'.
...
- full_menu: The dictionary that holds the full menu of prices.
"""
...
def create_full_menu(small_menu, multiplier):
    ...
    for item in small_menu:
        # Multiply the price by the multiplier and store it in full_menu
        for size in multiplier:
            full_menu[(item, size)] = small_menu[item] * multiplier[size]
...
"""This function, get_total_price, calculates the total price of buying each specification of a
...
- menu: The dictionary that holds the full menu of prices.
...
def get_total_price(menu, product):
    ...
    for (item, size) in menu:
        if product is None or size == product or item == product:
            tot += menu[(item, size)]
...
def get_average_price(menu, product):
    ...
    for (item, size) in menu:
        if size == product or item == product:
            tot += menu[(item, size)]
            len += 1
...
def set_price_sqrt(menu, product):
    ...
    for (item, size) in menu:
      if size == product or item == product:
       menu[item, size] = sqrt(menu[item, size])
...
"""  Strangely enough, some stores do not sell individual products.  They
  ...
  This function consults the dictionary that represents the menu and
    returns a dictionary which holds the total price for for all products along a given dimension.
...
def create_one_of_each(menu, axis):
    ...
    if axis == TYPE:
        for (item, size) in menu:
            if item not in new_menu:
                new_menu[item] = 0
            new_menu[item] = round(new_menu[item] + menu[item, size], 2)

    elif axis == SIZE:
        for (item, size) in menu:
            if size not in new_menu:
                new_menu[size] = 0
            new_menu[size] = round(new_menu[size] + menu[item, size], 2)
